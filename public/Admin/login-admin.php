  ...
  <div class="min-h-100 p-0 p-sm-6 d-flex align-items-stretch">
    <div class="card w-25x flex-grow-1 flex-sm-grow-0 m-sm-auto">
      <div class="card-body p-sm-5 m-sm-3 flex-grow-0">
        <h1 class="mb-0 fs-3" style="text-align: center;">Đăng nhập</h1>
        <div class="fs-exact-14 text-muted mt-2 pt-1 mb-5 pb-2"></div>
        <div class="mb-4"><label class="form-label">Tên tài khoản</label><input id="username" type="text" class="form-control form-control-lg" />
        </div>
        <div class="mb-4"><label class="form-label">Mật khẩu</label><input id="password" type="password" class="form-control form-control-lg" /></div>
        <div><button type="submit" id="btn_signin" class="btn btn-primary btn-lg w-100">Đăng nhập</button></div>
      </div>
    </div>
  </div>
  </div><!-- scripts -->
  ...
  $(document).ready(function() {
    $(document).keypress(function(event) {
      ...
      if (keycode == '13') {
        ...
        //Gửi ajax
        $.ajax({
          url: "Controller/ControllerSignin.php",
          type: "post",
          dataType: "text",
          data: {
            username: username,
